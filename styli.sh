...
declare DIR="" SUB="" ARTIST="" SAVE="" DEBUG="${DEBUG:-0}" LINK="unsplash"
...
unsplash() {
	if [ -n "$SEARCH" ]; then
		local KEYWORDS="${SEARCH// /+}"
		QRY="${KEYWORDS}"
	else
		QRY=""
	fi

	IMGID=$(exdbg curl -s "https://api.unsplash.com/photos/random?query=${QRY}&orientation=landscape&client_id=${UNSPLASH_ACCESS_KEY}")

	if [ -n "$HEIGHT" ] || [ -n "$WIDTH" ]; then
		# this has apparently changed, we need fit to guarantee correct resolution
		IMGURL=$(jq -r ".urls.raw + \"&w=${WIDTH}&h=${HEIGHT}&fit=crop\"" <<<"${IMGID}")
	else
		IMGURL=$(jq -r ".urls.raw" <<<"${IMGID}")
	fi

	echo "saving from unslpash ($IMGURL) to $WALLPAPER" | $NOTIFY_OUT

	exdbg curl -s -o "$WALLPAPER" "$IMGURL"
}
...
for f in "${FILTERS[@]}"; do
	if [ -n "$f" ]; then
		...
		if [[ $(type -t "${filtcmd[0]}") == function ]]; then
			...
			exdbg ${filtcmd[0]} ${filtcmd[@]:1} || echo "error executing ${filtcmd[0]} ${filtcmd[*]:1}" | $NOTIFY_ERR
		else
